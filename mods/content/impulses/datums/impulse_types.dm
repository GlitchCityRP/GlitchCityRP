...
/// A typecache of types that can be picked as a goal for the Get an Object impulse.
/// This will be removed in a future update and instead autogenerated based on items shelved in shops.
/datum/map/var/list/obj/item/obtain_objective_items // TODO: autogenerate based on store rack contents instead
...
/datum/impulse/obtain/can_roll()
	...
	if(length(global.using_map.obtain_objective_items) && !is_dense_assoc(global.using_map.obtain_objective_items))
		global.using_map.obtain_objective_items = typecacheof(global.using_map.obtain_objective_items)
	...
	if(!length(global.using_map.obtain_objective_items))
		can_roll_cached = FALSE
	return can_roll_cached

/datum/impulse/obtain/create()
	. = ..()
	var/rolled_type = pick(global.using_map.obtain_objective_items)
	if(!rolled_type)
		CRASH("Obtain impulse had no types to choose from but was rolled anyway!")
	target_item_type = rolled_type
